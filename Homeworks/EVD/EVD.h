...
public:
    ...
    EVD(matrix& M){
        ...
        V = identity(M.nrows); // This is a copy operator because identity only exists temporary
        w = pp::vec<double> (M.nrows); // This is a copy operator only exists temporary.
        // V.print("initial V=");
        /* run Jacobi rotations on A and update V */
        bool changed = true;
        while (changed){
            changed = false;
            for(int p=0;p<M.nrows-1;p++)
            for(int q=p+1;q<M.nrows;q++){
		    // std::cerr<< "p="<<p<<" q="<<q<<"\n";
