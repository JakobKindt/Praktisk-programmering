...
// matrix hessian_central(std::function<double(pp::vec)>& phi, pp::vec& x);
std::tuple<double, pp::vec, int> newton_central(std::function<double(pp::vec)>& phi, pp::vec x, double acc = 1e-3, int max_count = 1000);