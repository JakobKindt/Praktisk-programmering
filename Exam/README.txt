I got project 8 about particle swarm optimization (PSO) which I implemented in a class called PSO. To instantiate it, one simply provides a function, two vectors (from my own vector class) which contain the lower and upper bounds for where we limit the particles and an integer of how many particles we want. The algorithm then instantiates the particles by randomizing their initial positions and velocities which are kept in matrices along side with the optimal positions, i.e. the positions at which the function so far is minimal, for the individual particles and for the particles as a group. It works in any amount of dimensions.
